...
void LoginDialog::show() {
  ...
  if (profile->open()) {
    ...
    if (profile->getAutoLoginUser(autoLoginUser) && !autoLoginUser.isEmpty() &&
        preUsername.isEmpty()) {
    ...
    if (preUsername.isEmpty()) {
      ...
      if (profile->getLastLoginUser(lastLoginUsername) &&
          !lastLoginUsername.isEmpty()) {
...
void LoginDialog::set_interface_list(QStringList const &list) {
  ui->comboIface->clear();
...
void LoginDialog::set_device_name(const QString &name) {
  ...
  if (index >= 0)
    ui->comboIface->setCurrentIndex(index);
}

void LoginDialog::on_btnShowPassword_clicked(bool checked) {
  ui->editPassword->setEchoMode(checked ? QLineEdit::Normal
                                        : QLineEdit::Password);
}
