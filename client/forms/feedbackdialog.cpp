...
void FeedbackDialog::on_sldScore_sliderMoved(int position) {
  ...
  case 3:
    ...
    break;
  case 4:
    word = tr("用起来还算不错，但在某些功能上还需改善");
    color = QColor::fromRgb(100, 200, 20);
    break;
  case 5:
    word = tr("超级好用啊，给开发者点赞！");
...
void FeedbackDialog::on_btnSubmit_clicked() {
  ...
  QString uname = QStandardPaths::writableLocation(QStandardPaths::HomeLocation)
                      .section("/", -1, -1);
  QString mach = QHostInfo::localHostName();

  QString detail = this->ui->textDetail->toPlainText();
  QString star = QString::number(
      this->ui->chkToScore->isChecked() ? ui->sldScore->value() : 0);
  QString contact = ui->txtContactValue->text();
  if (contact.isEmpty())
    contact = "";
  else
    contact = ui->cmbContact->itemText(ui->cmbContact->currentIndex()) + " " +
              contact;
  ...
  QString urlStr =
      QString("https://%0/feedback/report.php").arg(NDR_FEEDBACK_SERVER);
  ...
  QByteArray postData =
      (QString("account=" + account.toUtf8().toPercentEncoding() +
               ...
               "&combo=" + combo.toUtf8().toPercentEncoding())
           .toUtf8());
...
void FeedbackDialog::postFinished() {
  ...
  QMessageBox::information(this, tr("反馈成功"),
                           tr("感谢您反馈，我们会考虑并尽力采纳您的意见"));
  return;
failed:
