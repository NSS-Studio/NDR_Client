  ...
  QObject::connect(ui->checkBox_2, &QCheckBox::clicked, [=]() {
    ...
    if (ui->checkBox_2->isChecked())
      ui->checkBox_2->setChecked(
          creatReg(autoStartRegRoot, autoSatrtKeyWay, autoStartkeyName,
                   QDir::currentPath() + tr("/ndr-client.exe")));
    else
      ui->checkBox_2->setChecked(
          !delReg(autoStartRegRoot, autoSatrtKeyWay, autoStartkeyName));
  });
#endif
#ifdef Q_OS_MAC
  ui->cmbHotKey->clear();
...
void SettingsDialog::on_buttonBox_accepted() {
  // config->setValue("Locale",
  // ui->cmbLanguage->itemData(ui->cmbLanguage->currentIndex()));
  this->accept();
}
...
bool SettingsDialog::getFormData(SettingsSet *settings) {
  ...
  if (savedLang.isEmpty())
    ui->cmbLanguage->setCurrentIndex(0); // Auto
  else if (savedLang == "zh_CN")
  ...
  if (ui->cmbLanguage->count() != 5) {
    for (auto const &fileName : langFileList) {
      ...
      if (fileName == savedLang)
        ui->cmbLanguage->setCurrentIndex(ui->cmbLanguage->count() - 1);
    }
  }
  ...
  if (this->exec() == QDialog::Accepted) {
    ...
    settings->language =
        this->ui->cmbLanguage->itemData(ui->cmbLanguage->currentIndex())
            .toString();
    return true;
  }
  return false;
...
bool SettingsDialog::chkWeb() { return this->ui->chkWebEnable->isChecked(); }

void SettingsDialog::on_chkAutoClose_pressed() {
  if (!this->ui->chkAutoClose->isChecked()) {
    QMessageBox::StandardButton re = QMessageBox::information(
        nullptr, "警告",
        "勾选自动关闭通知有可能导致你无法看到来自学校的某些重要通知\n"
        "请再次确认是否关闭",
...
bool SettingsDialog::isRunAsAdmin() {
  ...
  if (GetTokenInformation(hToken, TokenElevation, &tokenEle, sizeof(tokenEle),
                          &dwRetLen)) {
...
bool SettingsDialog::creatReg(HKEY proxyRoot, const QString &keyWay,
                              const QString &keyName, const QString &keyValue) {
  HKEY hKey;
  REGSAM flag = KEY_CREATE_SUB_KEY | KEY_READ | KEY_WRITE | KEY_SET_VALUE;

  LONG ret = RegOpenKeyEx(proxyRoot, LPCTSTR(keyWay.unicode()), 0,
                          KEY_WOW64_64KEY | flag, &hKey);
  if (ret != ERROR_SUCCESS)
    ret = RegOpenKeyEx(proxyRoot, LPCTSTR(keyWay.unicode()), 0,
                       KEY_WOW64_32KEY | flag, &hKey);
  if (ret != ERROR_SUCCESS) {
    qDebug() << "Open reg failed: " << ret;
    message(dialogKinds::warning,
            "设置开机自启动失败，错误代码: " + QString::number(ret, 10));
    return false;
  }
  ...
  ret = RegSetValueEx(
      hKey, LPCTSTR(keyName.unicode()), 0, REG_SZ, LPBYTE(pchrName),
      wcslen((TCHAR *)pchrName) * sizeof(TCHAR) + 1); //设置注册表项
  if (ret != ERROR_SUCCESS) {
...
bool SettingsDialog::delReg(HKEY proxyRoot, const QString &keyWay,
  ...
  if (ret != ERROR_SUCCESS) {
    ...
    message(dialogKinds::warning,
            "取消开机自启动失败，错误代码: " + QString::number(ret, 10));
    return false;
  }
...
QMessageBox::StandardButton SettingsDialog::message(dialogKinds question,
                                                    const QString &text) {
  ...
  case dialogKinds::question:
    reply = QMessageBox::question(this, tr("Question"), text,
  ...
  case dialogKinds::error:
    reply = QMessageBox::critical(this, tr("Error"), text,
