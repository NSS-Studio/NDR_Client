...
HangupDialog::HangupDialog(QWidget *parent)
    : QDialog(parent), ui(new Ui::HangupDialog) {
  ...
  // ui->label->setText(tr("NDR
  // 已经将新版本软件包下载到了临时目录，请打开目录并手动安装。如果您不想立即更新，请选择下次提示的时间并单击“%1”").arg(ui->btnDelay->text()));
#endif
}
...
bool HangupDialog::event(QEvent *e) {
  if (e->type() == QEvent::KeyPress &&
      (dynamic_cast<QKeyEvent *>(e))->key() == Qt::Key_Escape) {
    return true;
  }
...
void HangupDialog::on_btnAccept_clicked() { loop.quit(); }

void HangupDialog::setMessage(const QString &message) {
  ui->label->setText(
      QString("<p style=\"font-size:8pt; color:#000000;\">%1</p>")
          .arg(message));
}
