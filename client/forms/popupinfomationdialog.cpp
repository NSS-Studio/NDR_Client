...
PopUpInfomationDialog::PopUpInfomationDialog() : QObject{nullptr} {
  ...
  history =
      new QSettings(utils::appHome + "/history.ini", QSettings::IniFormat);
  historyNum = 0;
  QString message;
  do {
    message =
        history->value(QString::number(historyNum, 10), "none").toString();
    text.insert(message);
    historyNum++;
...
PopUpInfomationDialog::~PopUpInfomationDialog() { popUp->deleteLater(); }
...
void PopUpInfomationDialog::getXmlFromNSS(const QString &url) {
  ...
  if (flag) {
    ...
    connect(reply, &QNetworkReply::finished, this,
            &PopUpInfomationDialog::getXmlDone);
  }
}
  ...
  if (doc.setContent(xml, true)) {
    ...
    if (status == "has") {
      ...
      for (int i = 0; i < nodes.size(); i++) {
        ...
        for (QDomNode j = node.firstChild(); !j.isNull() && k < 4;
             j = j.nextSibling()) {
        ...
        if (temp[0] == "1" && !temp[3].isEmpty()) {
          if (text.find(temp[3]) == text.constEnd()) {
            text.insert(temp[3]);
            group.push_back(
                message{bool(temp[0].toInt()), temp[1], temp[2], temp[3]});
...
void PopUpInfomationDialog::showMessage() {
  ...
  connect(nextPage, &QPushButton::clicked, this,
          &PopUpInfomationDialog::goNextPage);
  connect(prePage, &QPushButton::clicked, this,
          &PopUpInfomationDialog::goPrePage);
...
void PopUpInfomationDialog::resetTime() { timePass = 0; }
...
void PopUpInfomationDialog::goNextPage() {
  ...
  page->setText(tr("%0/%1").arg(pageNow).arg(group.size()));
...
bool PopUpInfomationDialog::checkWindow() {
  ...
  qDebug() << winSize.right - winSize.left << " "
           << winSize.bottom - winSize.top << endl;
