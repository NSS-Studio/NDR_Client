...
BaseDsl::BaseDsl(const QString &name, QObject *parent) : QObject(parent) {
    this->name = name;
    connection = nullptr;
    ppp_service = nullptr;
    preferences = nullptr;
...
bool BaseDsl::dial(const QString &username, const QString &password,
                   const QString &device_name, QString &errorMsg) {
    // qDebug() << "device_name" << device_name;
#define SET_ERROR_MSG(S)                                                       \
    errorMsg = QString("%1: %2").arg(S).arg(SCErrorString(SCError()))
    ...
    /*
       ...
       conn_cfg.value("Device/Interface").toByteArray();
    ...
    */
    ...
    CFStringRef name_cf = CFStringCreateWithCString(nullptr, name_ba.data(),
    ...
    CFTypeRef ppp_values[] = {
        CFStringCreateWithCString(nullptr, username_ba.data(),
                                  kCFStringEncodingUTF8),
        CFStringCreateWithCString(nullptr, password_ba.data(),
                                  kCFStringEncodingUTF8),
        kCFBooleanFalse,
        kCFBooleanFalse,
        kCFBooleanTrue,
    ...
    CFTypeRef values[] = {interface_dict, ip4_dict, ip6_dict, ppp_dict,
                          name_cf};
    ...
    if (!preferences) {
        AuthorizationFlags root_flags = kAuthorizationFlagDefaults |
                                        kAuthorizationFlagExtendRights |
                                        kAuthorizationFlagInteractionAllowed |
                                        kAuthorizationFlagPreAuthorize;
        AuthorizationRef auth;
        if (AuthorizationCreate(nullptr, kAuthorizationEmptyEnvironment,
                                root_flags, &auth) == noErr) {
            preferences = SCPreferencesCreateWithAuthorization(
                nullptr, CFSTR("NSS"), nullptr, auth);
        } else {
