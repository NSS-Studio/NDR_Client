...
bool MacOsBaseDsl::dial(const QString &username, const QString &password,const QString &device_name, QString &errorMessage) {
    ...
    if (!SCNetworkServiceEstablishDefaultConfiguration(ppp_service)) {
        setErrorMessage(errorMessage, "SCNetworkServiceEstablishDefaultConfiguration");
        return false;
    }
    SCNetworkSetRef set = SCNetworkSetCopyCurrent(preferences);
    if (!set) {
...
bool MacOsBaseDsl::isDisconnected()
{
    if (!connection)
        return true;
    qDebug() << "connection status = " << SCNetworkConnectionGetStatus(connection);
    ...
    qDebug() << __PRETTY_FUNCTION__ <<" "<<"ppp status:" <<  pppStatusValue;
...
QStringList MacOsBaseDsl::getAvailableInterfaces() {
    ...
    for (CFIndex index = 0; index < service_count; ++index) {
        SCNetworkServiceRef service =
            static_cast<SCNetworkServiceRef>(CFArrayGetValueAtIndex(services, index));
        SCNetworkInterfaceRef iface = SCNetworkServiceGetInterface(service);
